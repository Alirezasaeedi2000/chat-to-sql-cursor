...
from vector import VectorStoreManager, QueryExpander
...
class TestQueryHistory(unittest.TestCase):
    ...
    def test_search_history(self):
        ...
        self.history.add_entry(
            "Count orders", "SELECT COUNT(*) FROM orders", "SHORT_ANSWER"
        )
...
class TestOverridesAndProfiles(unittest.TestCase):
    """Tests for COMBO mode overrides and aggregated employee profiles."""

    @patch("query_processor.create_engine_from_url")
    def test_phrase_combo_override_and_profile_aggregation(self, mock_create_engine):
        """Ensure phrase override forces COMBO and aggregation yields single-row profile."""
        ...
        # Aggregated single-row profile DF
        ...
        with patch("pandas.read_sql") as mock_read_sql:
            # Return a DF for both EXPLAIN and SELECT paths
            mock_read_sql.return_value = profile_df

            # Mock LLM so that k-best generated candidates are invalid (non-SELECT),
            # ensuring the deterministic aggregated profile candidate is chosen.
            with patch("query_processor.ChatOllama") as mock_llm_class:
                mock_llm = Mock()
                mock_llm_class.return_value = mock_llm
                bad_sql_resp = Mock()
                bad_sql_resp.content = "```sql\nUPDATE t SET x=1\n```"
                # Only candidate generation calls occur (no mode call due to override)
                mock_llm.invoke.side_effect = [bad_sql_resp, bad_sql_resp, bad_sql_resp]

                vector_manager = VectorStoreManager()
                processor = QueryProcessor(mock_engine, vector_manager)

                # Phrase override enforces COMBO, and profile aggregator kicks in.
                result = processor.process("use combo mode analyze employee id 1363")

                self.assertEqual(result.mode, "COMBO")
                self.assertEqual(result.metadata.get("row_count"), 1)
                self.assertIsNotNone(result.short_answer)

    def test_employee_profile_sql_builder(self):
        """The SQL builder for employee profile should emit aggregated query with GROUP_CONCAT."""
        mock_engine = Mock()
        vector_manager = VectorStoreManager()
        processor = QueryProcessor(mock_engine, vector_manager)
        sql = processor._maybe_build_employee_profile("analyze employee id 1363")
        self.assertIsNotNone(sql)
        self.assertIn("GROUP_CONCAT", sql)
        self.assertIn("FROM `employe`", sql)
        self.assertIn("WHERE e.id = 1363", sql)
