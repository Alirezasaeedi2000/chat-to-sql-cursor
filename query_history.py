...
class QueryHistoryManager:
    ...
    def __init__(
        self, history_file: str = "outputs/query_history.json", max_entries: int = 1000
    ):
    ...
    def _save_history(self) -> None:
        ...
        try:
            ...
            with open(self.history_file, "w", encoding="utf-8") as f:
                json.dump(
                    [asdict(entry) for entry in self.history],
                    f,
                    indent=2,
                    ensure_ascii=False,
                )
    ...
    def add_entry(
        ...
        entry_id = f"q_{datetime.now(UTC).strftime('%Y%m%d_%H%M%S_%f')}"
    ...
    def search_history(
        ...
        for entry in reversed(self.history):  # Most recent first
            ...
            if query:
                if (
                    query_lower not in entry.user_query.lower()
                    and (
                        not entry.sql_query
                        or query_lower not in entry.sql_query.lower()
                    )
                    and not any(query_lower in tag.lower() for tag in entry.tags)
                ):
                    continue
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        exec_times = [
            e.execution_time_ms for e in self.history if e.execution_time_ms is not None
        ]
    ...
    def get_popular_queries(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        popular = sorted(query_groups.items(), key=lambda x: len(x[1]), reverse=True)[
            :limit
        ]
            ...
            result.append(
                {
                    ...
                    "success_rate": sum(1 for e in entries if e.error is None)
                    / len(entries),
                }
            )
